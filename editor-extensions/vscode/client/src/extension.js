...
async function startClient(workspace_uri) {
    ...
    if (workspace_uri) {
        ...
        const absoluteBuildScript = path.join(workspaceFolder + "/" + buildScriptPath);
    ...
    const client = new node_1.LanguageClient("grain", "Grain Language Server", serverOptions, clientOptions);
    await client.start();
    const grainDocCompletionProvider = new GrainDocCompletionProvider_1.GrainDocCompletionProvider(client);
    vscode_1.languages.registerCompletionItemProvider("grain", grainDocCompletionProvider, "*");
    return client;
}
;
const restart = () => {
