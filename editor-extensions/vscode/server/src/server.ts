...
connection.onDocumentFormatting((handler) => {
	...
	if (uri.scheme == fileScheme) {
		let filename = filenameFromUri(uri);
		connection.console.log('documentFormatting');
		connection.console.log(filename);
		...
		try {
			...
			if (textDocument != undefined) {
				...
				let result_buffer = childProcess.execFileSync(
					"grain",
					["format"],
					{ input: text, cwd }
				);
				...
				return Promise.resolve([{
					range: {
						...
						end: Position.create(Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER)
					},
					newText: res
				}]);
